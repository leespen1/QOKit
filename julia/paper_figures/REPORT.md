...
All scripts are in `julia/paper_figures/` and use CairoMakie for plotting.
Output goes to `julia/paper_figures/output/`.
...
have been added to `julia/src/linear_ramp.jl` and exported from JuliaQAOA.
...
### `julia/src/linear_ramp.jl` — Added to JuliaQAOA Module
