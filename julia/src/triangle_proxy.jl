"""
The original, hard-coded triangle proxy we used.

This is now obsolete, since the parameterized version `TriangleProxy` has the
same behavior when the default tunable parameters are used.
"""
