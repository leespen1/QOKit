...
def fit_multiple_graphs(num_graphs, num_nodes, edge_probability=0.5, graph_type="erdos_renyi", ws_num_neighbors=2):
    ...
    assert graph_type in ["erdos_renyi", "barabasi_albert", "watts_strogatz"], "graph_type must be one of 'erdos_renyi', 'barabasi_albert', or 'watts_strogatz'"
    ...
    fitted_params, fitmse1 = fit_proxy_to_real(
        opt_init_proxy, homodist, initial_params, bounds, max_iter=10000, fail_til_shrink=50, fail_til_end=100, grid_size_start=9
    )
    ...
    fitted_result = QAOA_proxy_optimize_gamma_beta(
        fitted_proxy, gamma_0, beta_0, optimizer_method="Nelder-Mead", optimizer_options={"maxiter": 10000, "epsilon": 0.00001}
    )
    ...
    print(
        f"With {num_nodes} nodes, {num_graphs} graphs, edge probability {edge_probability}, \n" f"Fitted parameters were {[round(x,2) for x in fitted_params]}"
    )
...
def fit_multiple_graphs_normal(graphs, num_nodes, edge_probability, graph_type, ws_num_neighbors=None):
    ...
    def mse_dist_loss_normal(params, homodist):
        proxy = NormalProxy(num_constraints=num_constraints, num_qubits=num_qubits, cost_mean=params[0], cov_1=params[1], cov_2=params[2])
        return mse_dist_loss(proxy, homodist)

    print("\nFitting normal proxy to homogeneous distribution ...")

    result = minimize(
        mse_dist_loss_normal, initial_params, args=(homodist,), method="Nelder-Mead", bounds=bounds, options={"maxiter": 10000, "epsilon": 0.0001}
    )
    fitted_params = result.x
...
def evaluate_paper_proxy(graphs, num_nodes, edge_probability, graph_type, ws_num_neighbors=None):
    ...
    paper_proxy = PaperProxy(num_constraints=num_constraints, num_qubits=num_qubits, prob_edge=edge_probability)

    # Optimize gamma and beta for the paper proxy
    gamma_0 = np.array([0.2])
    beta_0 = np.array([0.1])
    paper_result = QAOA_proxy_optimize_gamma_beta(
        paper_proxy, gamma_0, beta_0, optimizer_method="Nelder-Mead", optimizer_options={"maxiter": 10000, "epsilon": 0.00001}
    )
...
if __name__ == "__main__":
    ...
    df, graphs = fit_multiple_graphs(num_graphs=num_graphs, num_nodes=startnodes, edge_probability=edge_probability, graph_type=graph_type)
    ...
    for num_nodes in range(startnodes + 1, endnodes + 1):
        ...
        df_new, graphs_new = fit_multiple_graphs(num_graphs=num_graphs, num_nodes=num_nodes, edge_probability=edge_probability, graph_type=graph_type)
    ...
    for idx, row in df.iterrows():
        ...
        paper_proxy = PaperProxy(num_constraints=config_max_edges, num_qubits=int(row["num_nodes"]), prob_edge=float(row["edge_probability"]))
