...
def QAOA_proxy(
    ...
    amplitude_proxies = np.zeros(
        (num_QAOA_layers + 1, num_unique_costs), dtype=np.complex128
...
def QAOA_proxy_expectation(proxy, amplitude_proxies: np.ndarray, drop_first_N_costs: int = 0) -> float:  # 1D array  # Optionally, ignore the lowest costs
    """
    ...
    # If proxy is a NJIT-compiled obj, use njit version
    if type(proxy).__module__ == "numba.experimental.jitclass.boxing":
        proxy_expectation_value = QAOA_proxy_expectation_njit(proxy, amplitude_proxies, drop_first_N_costs)
        return proxy_expectation_value
    # If proxy is is a Juliacall obj, use julia version
    elif type(proxy).__module__ == "juliacall":
        # Convert result to numpy array, and take transpose (Because Julia is column-major, while Numpy is row-major)
        proxy_expectation_value = jl.QAOA_proxy_expectation(proxy, amplitude_proxies, drop_first_N_costs)
        return proxy_expectation_value
...
def QAOA_proxy_expectation_from_gamma_beta(
    ...
    expectation = QAOA_proxy_expectation(proxy, final_amplitude_proxies)
...
def QAOA_proxy_optimize_gamma_beta(
    ...
    result = scipy.optimize.minimize(
        inverse_proxy_objective_function(proxy, num_QAOA_layers, expectations),
        init_freq,
        args=(),
...
def compute_amplitude_sum(
    ...
    # If proxy is a NJIT-compiled obj, use njit version
    if type(proxy).__module__ == "numba.experimental.jitclass.boxing":
        amp_sum = compute_amplitude_sum_njit(proxy, prev_amplitudes, gamma, beta, cost_1)
        return amp_sum
    ...
    for cost_2 in range(num_unique_costs):
        for distance in range(proxy.num_qubits + 1):
            ...
            num_costs_at_distance = proxy.N_cost_distance_distribution(cost_1, distance, cost_2)
...
@njit
def QAOA_proxy_expectation_njit(proxy, amplitude_proxies: np.ndarray, drop_first_N_costs: int = 0) -> float:  # Optionally, ignore the lowest costs
...
def inverse_proxy_objective_function(proxy, num_QAOA_layers: int, expectations: list[np.ndarray] | None) -> typing.Callable:
    """
    ...
    if type(proxy).__module__ == "numba.experimental.jitclass.boxing":
        ...
        inverse_objective = jl.inverse_proxy_objective_function(proxy, num_QAOA_layers, expectations)
        return inverse_objective
