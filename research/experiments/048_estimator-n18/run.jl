...
E047 established a depth-split verdict on two sizes (p=3 deficit vs the
enumeration sampler ~0.005 pooled at n<=14, +0.022 at n=16) and its README
flags the trend as resting on two points. This run adds n=18 (statevector
truth at 2^18 still computable): exact / samp10 / poly(S=10,K=200), both
objectives, both depths, 7 families x 3 instances. Same seeds and grids as
E044/E046/E047; part label "D". Derived from E047's run.jl (part B only,
timing part removed).

Run:   JULIA_NUM_THREADS=auto julia --project research/experiments/048_estimator-n18/run.jl  (~2-3 h)
Smoke: E48_SMOKE=1 JULIA_NUM_THREADS=auto julia --project research/experiments/048_estimator-n18/run.jl
=#
...
const SMOKE = get(ENV, "E48_SMOKE", "0") == "1"
const SEED  = 20260611
const INST  = SMOKE ? 2 : 3
...
const B_N  = SMOKE ? 12 : 18   # smoke size must be even (3-regular family)
...
function main()
    ...
    open(respath, "w") do io
        ...
        for r in rows_d, line in r; println(io, line); end
    end
