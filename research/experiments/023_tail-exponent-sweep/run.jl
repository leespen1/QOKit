#=
E023 — Where is the crossover? A tail-exponent sweep.

E022 found the ranking inversion at Pareto(α=1.5) and its absence at
Exp(1). A referee will ask whether α=1.5 is cherry-picked. This sweep
locates the boundary: Pareto α ∈ {1.2, 1.5, 2.0, 3.0, 5.0} (mean-weight-
rescaled grids as in E022, so transfer keeps its practitioner's fix),
both families at n=16, p=1 and p=3, 10 instances per cell.

Output: results_task<ID>.csv (family,n,instance,seed,m,law,method,p,ar,ceil,regret).
Submit:  cd research/experiments/023_tail-exponent-sweep && sbatch run.sb
Smoke:   E23_SMOKE=1 julia --project research/experiments/023_tail-exponent-sweep/run.jl
=#
...
const ALPHAS = Dict("a1.2" => 1.2, "a1.5" => 1.5, "a2.0" => 2.0,
                    "a3.0" => 3.0, "a5.0" => 5.0)
...
function main()
    ...
    fam, n, law = SMOKE ? ("ER(0.5)", 12, "a1.5") : TASKS[tid == 0 ? 1 : tid]
    draw = LAWS[law]
    ...
    println("E023 task complete → $outpath")
end
