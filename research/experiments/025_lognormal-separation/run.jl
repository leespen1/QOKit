#=
E025 — Tail weight or infinite variance? The lognormal separation.

E023/E024 put the transfer↔proxy crossover at Pareto α≈2 — the
infinite-variance point. But Pareto confounds "heavy tail" with "infinite
variance". Lognormal(0, σ) weights always have finite variance while their
tails (and max-to-sum edge dominance) grow without bound in σ. If the
crossover appears at large σ anyway, the mechanism is effective
single-edge dominance; if transfer holds at every σ, the variance story
sharpens. σ ∈ {0.5, 1.0, 1.5, 2.0, 2.5}, both families, n=16, p=1 and
p=3, 10 instances — otherwise E023's harness verbatim.

Output: results_task<ID>.csv (family,n,instance,seed,m,law,method,p,ar,ceil,regret).
Submit:  cd research/experiments/025_lognormal-separation && sbatch run.sb
Smoke:   E25_SMOKE=1 julia --project research/experiments/025_lognormal-separation/run.jl
=#
...
const SIGMAS = Dict("s0.5" => 0.5, "s1.0" => 1.0, "s1.5" => 1.5,
                    "s2.0" => 2.0, "s2.5" => 2.5)
const TASKS = [(f, 16, l) for l in sort(collect(keys(SIGMAS)))
...
function main()
    ...
    fam, n, law = SMOKE ? ("ER(0.5)", 12, "s2.0") : TASKS[tid == 0 ? 1 : tid]
    σw = SIGMAS[law]
    ...
    for f in keys(GENS)
        ...
        for rep in 1:N_SOURCES
            ...
            sw = exp.(σw .* randn(srng, length(sedges)))
    ...
    println("E025 task complete → $outpath")
end
