#=
E022 — Strong weight heterogeneity, with the practitioner's fair transfer.

E021 (i.i.d. U[0,1] weights) eroded transfer 3–10× but no crossover. Here
the heterogeneity is turned up — Exp(1) and heavy-tailed Pareto(α=1.5,
infinite variance) edge weights — and transfer gets its practitioner's fix
for free: every instance's γ-grid is rescaled by its own mean weight
(γ = γ̂/w̄ over a shared normalized grid γ̂), so transferring a grid INDEX
is exactly mean-weight-rescaled angle transfer. The proxy only wins if
per-instance structure beyond overall scale matters. Either outcome closes
the practitioner map.

Cells: (ER(0.5), 3-regular) × n ∈ {14, 16} × law ∈ {exp, pareto15},
10 instances. Depths: p=1 (γ̂ ∈ [0, 2π] × β ∈ [0, π/2], 40×40) and p=3
ramps (γ̂ ∈ [0.1, 3.2], β ∈ [0.05, 0.8], 8⁴).

Methods per instance (all on the instance's own scaled grid = the
ceiling's grid; regret ≥ 0 by construction):
  ceiling          — true weighted grid ceiling (threaded CPU statevector)
  binned_exact     — binned proxy, K=64, exact bin-label homodist
  binned_sampled   — same, N from S=10 samples per bin
  transfer_1..3    — grid-INDEX transfer from an n=10 same-family source
                     with its own weights (≡ mean-weight-rescaled transfer)
  universal        — pooled index argmax over all 6 sources

Output: results_task<ID>.csv (long; family,n,instance,seed,m,law,method,p,ar,ceil,regret).
Submit:  cd research/experiments/022_heavy-tail-weights && sbatch run.sb
Smoke:   E22_SMOKE=1 julia --project research/experiments/022_heavy-tail-weights/run.jl
=#
...
const LAWS = Dict("exp" => r -> -log(1 - r), "pareto15" => r -> (1 - r)^(-1/1.5))
const TASKS = [(f, n, l) for l in ("exp", "pareto15")
                          for n in (14, 16), f in ("ER(0.5)", "3-regular")] |> vec
const GENS = Dict(
...
function main()
    ...
    println("E022 task complete → $outpath")
end
