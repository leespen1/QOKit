#=
E030 — Free refinement: does polishing on the PROXY's landscape transfer?

E029: continuous refinement on the TRUE landscape erases the deep
high-dispersion corner from any start — but costs ~2000 true evaluations,
unaffordable where evaluations are shots. The proxy's landscape is free
and continuous. Here: compass-refine the ramp endpoints (then all 2p
angles) maximizing the BINNED PROXY's predicted objective, starting from
the proxy's own grid choice, and spend exactly ONE true evaluation on the
result. Compare true AR: grid choice vs proxy-refined vs E029's
true-refined optimum. Same two cells/instances as E028/E029.

Output: results_task<ID>.csv
(family,n,instance,seed,m,law,stage,ar,regret_vs_gridceil)
Submit:  cd research/experiments/030_proxy-side-refinement && sbatch run.sb
Smoke:   E30_SMOKE=1 julia --project research/experiments/030_proxy-side-refinement/run.jl
=#
...
const SMOKE = get(ENV, "E30_SMOKE", "0") == "1"
...
function main()
    ...
    for inst in 1:NINST
        ...
        pidx = binned_proxy_argmax(N_samp, binmeans, binsizes, n, scheds)
        c0 = combos[pidx]

        fP_ramp(θ) = binned_proxy_value(N_samp, binmeans, binsizes, n,
                                        linear_ramp(θ[1], θ[2], θ[3], θ[4], P)...)
        fP_full(θ) = binned_proxy_value(N_samp, binmeans, binsizes, n,
                                        θ[1:P], θ[(P + 1):(2P)])
        ...
        θf, _ = compass_search(fP_full, vcat(γr, βr),
                               vcat(fill(0.05 / w̄, P), fill(0.05, P)))

        stages = [("grid", tv[pidx] / c_opt),
                  ("proxy_ramp4", qaoa_expectation(costs, n, γr, βr) / c_opt),
                  ("proxy_full2p", qaoa_expectation(costs, n, θf[1:P], θf[(P+1):(2P)]) / c_opt)]
        for (stage, ar) in stages
            push!(rows, join(Any[fam, n, inst, seed, m, law, stage, ar,
                                 gridceil - ar], ","))
            println("  inst=$inst $stage ar=$(round(ar; digits=4))")
        end
        flush(stdout)
    end
    ...
    open(outpath, "w") do io
        println(io, "family,n,instance,seed,m,law,stage,ar,regret_vs_gridceil")
        foreach(r -> println(io, r), rows)
    end
    println("E030 task complete → $outpath")
end
