# E041 — Does the recommended recipe compose? Sampled N + normalized objective
...
unnormalized; E036/E040: normalization, exact N) promise?
...
The exact-N column reproduces E036/E040 exactly (p=3: 0.080 -> 0.044; p=1
...
regularizer against the proxy's winner's curse (E038): the systematic
...
- Grid ceilings, not continuous optima (E034 showed these are tight).
...
- One sampling draw per instance (rng seed+777). The 140 instances make the
  *average* effect solid, but whether the gain comes from noise per se or
  ...
  is NOT distinguished here — that is the E042 mechanism question. Do not
  put the "sampled beats exact" claim in the paper until E042 resolves it.

## Repro

```bash
JULIA_NUM_THREADS=auto julia --project research/experiments/041_sampled-plus-normalized/run.jl   # ~15 min
E25_SMOKE=1 julia --project research/experiments/041_sampled-plus-normalized/run.jl              # quick check
```
