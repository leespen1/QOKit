...
def write_hdf5(filepath, Nc_list, args_dict, seeds):
    ...
    - /seeds: 1D array of graph seeds
    - Attributes: all parameters from args_dict
    - Key attributes: graphType, numNodes, edgesPerNode
    """
    ...
    with h5py.File(filepath, "w") as f:
        ...
        f.attrs["edgesPerNode"] = args_dict["edgesPerNode"]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        ...
        description="Collects the number of occurences of each cost N(c) for Barabasi-Albert graphs.",
    )
    ...
    parser.add_argument("-m", "--edgesPerNode", type=int, nargs="+", required=True, help="Number of edges to attach from a new node to existing nodes (must satisfy 1 <= m < n). Supports multiple values.")
    ...
    try:
        ...
        all_combos = list(itertools.product(args.edgesPerNode, args.numNodes))
        ...
        for idx, (m, n) in enumerate(combos):
            if num_combos > 1:
                print(f"\n{'='*60}")
                print(f"Parameter combination {idx + 1}/{num_combos}: n={n}, m={m}")
                print(f"{'='*60}")
            single_args = argparse.Namespace(
                numNodes=n, edgesPerNode=m,
