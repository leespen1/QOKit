...
if __name__ == "__main__":
    ...
    parser.add_argument("-k", "--nearestNeighbors", type=int, nargs="+", required=True, help="Each node is joined with its k nearest neighbors in a ring topology (supports multiple values).")
    parser.add_argument("-p", "--rewiringProbability", type=float, nargs="+", required=True, help="Probability of rewiring each edge (supports multiple values).")
    ...
    try:
        ...
        all_combos = list(itertools.product(args.nearestNeighbors, args.rewiringProbability, args.numNodes))
        ...
        for idx, (k, p, n) in enumerate(combos):
            if num_combos > 1:
                print(f"\n{'='*60}")
                print(f"Parameter combination {idx + 1}/{num_combos}: n={n}, k={k}, p={p}")
                print(f"{'='*60}")
            single_args = argparse.Namespace(
                numNodes=n, nearestNeighbors=k, rewiringProbability=p,
