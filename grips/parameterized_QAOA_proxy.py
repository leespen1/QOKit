...
# TODO | Add julia version of this. Might be a good idea to have a struct
#        holding the parameters (but also less NJIT compatible)
def parameterized_QAOA_proxy(
    p: int,
    gamma: np.ndarray,
    beta: np.ndarray,
    num_constraints: int,
    ...
    return parameterized_QAOA_proxy_python(p, gamma, beta, num_constraints, num_qubits, h_tweak_sub, hc_tweak_add, l_tweak_mul, r_tweak_mul, terms_to_drop_in_expectation)
...
@njit
def parameterized_QAOA_proxy_python(
    p: int,
    gamma: np.ndarray,
    beta: np.ndarray,
    num_constraints: int,
