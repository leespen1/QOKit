...
#=
This package lives at the repo root, which is also a DrWatson project. From
Julia, activate it the DrWatson way:

using DrWatson
@quickactivate "JuliaQAOA"
using JuliaQAOA

To make the exported functions available in Python via juliacall, activate the
repo-root project (the directory containing this package's Project.toml). The
grips bridge (python/grips/QAOA_proxy_interface.py) does this automatically;
done by hand from a script under python/grips/ it looks like:

from juliacall import Main as jl
jl.seval('''
using Pkg
Pkg.activate(joinpath(@__DIR__, "..", ".."))   # repo root, two levels up
Pkg.instantiate()
using JuliaQAOA
''')
